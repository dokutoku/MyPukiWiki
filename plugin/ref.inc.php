...
if (!defined('FILE_ICON')) {
	define('FILE_ICON', '<img src="'.IMAGE_DIR.'file.png" width="20" height="20" alt="file" style="border-width:0" />');
}
...
// 'left', 'center', 'right'
define('PLUGIN_REF_DEFAULT_ALIGN', 'left');

// Text wrapping
// true, false
define('PLUGIN_REF_WRAP_TABLE', false);
...
// false or true
define('PLUGIN_REF_DIRECT_ACCESS', false);
...
function plugin_ref_body(array $args) : array
{
	...
	*/
	...
	foreach (['right', 'left', 'center'] as $align) {
		if ($params[$align]) {
			$params['_align'] = $align;
...
function plugin_ref_action() : array
{
	...
	if ((!isset($vars['page'])) || (!isset($vars['src']))) {
		return ['msg'=>'Invalid argument', 'body'=>$usage];
	}
	...
	if (!file_exists($ref)) {
		return ['msg'=>'Attach file not found', 'body'=>$usage];
	}

	$is_image = preg_match(PLUGIN_REF_IMAGE, $filename);

	if (!$is_image) {
		return ['msg'=>'Seems not an image', 'body'=>$usage];
	}
