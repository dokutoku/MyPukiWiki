...
// 1: above 0: below
define('PLUGIN_COMMENT_DIRECTION_DEFAULT', '1');
...
function plugin_comment_convert()
{
	...
	if (in_array('noname', $options, true)) {
		...
		$nametags = '<label for="_p_comment_name_'.$comment_no.'">'.$_btn_name.'</label><input type="text" name="name" id="_p_comment_name_'.$comment_no.'" size="'.PLUGIN_COMMENT_SIZE_NAME.'" />'."\n";
	}

	$nodate = (in_array('nodate', $options, true)) ? ('1') : ('0');
