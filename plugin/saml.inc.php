...
function plugin_saml_action()
{
	...
	if (isset($vars['sso'])) {
		...
		if (!empty($errors)) {
			return ['msg'=>'SAML Error', print_r('<p>'.implode(', ', $errors).'</p>')];
		}
		...
		return ['msg'=>'SAML sign in', 'body'=>'SAML Sined in. but no redirection'];
	} elseif (isset($vars['sls'])) {
		...
		return ['msg'=>'SAML sign out', 'body'=>'SAML Sined out. '.$msg];
	} elseif (isset($vars['metadata'])) {
	...
	return ['msg'=>'Error', 'body'=>'SAML Invalid state srror'];
}
